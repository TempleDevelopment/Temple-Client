...
public class NameTags extends Module {
    ...
    private final IntSetting borderRed = new IntSetting("Border Red", this, 0, 255, 255);
    private final IntSetting borderGreen = new IntSetting("Border Green", this, 0, 255, 255);
    private final IntSetting borderBlue = new IntSetting("Border Blue", this, 0, 255, 255);
    ...
    public NameTags() {
        ...
        registerSettings(self, customFont, items, border, borderRed, borderGreen, borderBlue);
    }
    ...
    public void onRender(RenderLivingEvent.Specials.Pre<?> event) {
        ...
        if(e instanceof EntityPlayer && ((this.self.booleanValue() && mc.gameSettings.thirdPersonView != 0) || !e.equals(mc.player)) && !e.isDead && e.getHealth() > 0 && !e.isInvisible()) {
    ...
    public void onRenderWorld(float partialTicks) {
        for(EntityPlayer player : this.players) {
            if(((!this.self.booleanValue() || mc.gameSettings.thirdPersonView == 0) && player.equals(mc.player)) || player.isDead || player.getHealth() <= 0 || player.isInvisible()) {
                continue;
            }
    ...
    private void renderNametag(EntityLivingBase entity, Vec3d pos) {
        ...
        final int fontHeight = (int) Math.ceil(FontUtils.getFontHeight(this.customFont.booleanValue()));
        ...
        FontUtils.drawString(name, -totalWidth, 0, 0xFFFFFF, true, this.customFont.booleanValue());
        FontUtils.drawString(health, -totalWidth + this.getStringWidth(name), 0, this.getHealthColor(entity), true, this.customFont.booleanValue());
        FontUtils.drawString(ping, -totalWidth + this.getStringWidth(name + health), 0, this.getPingColor(entity), true, this.customFont.booleanValue());
    ...
    private void renderItem(ItemStack stack, int x, int y) {
        ...
        if(durability >= 0 && stack.getItem().showDurabilityBar(stack)) {
            ...
            FontUtils.drawString(duraString, x, y + (-y - 10), stack.getItem().getRGBDurabilityForDisplay(stack), true, this.customFont.booleanValue());
        }
    ...
    private void drawRoundedBorderedRect(int left, int top, int right, int bottom, int insideColor, EntityLivingBase entity) {
        ...
        int outsideR, outsideG, outsideB;
        if (TempleClient.friendManager.isFriend(entity.getName())) {
            outsideR = 0;
            outsideG = 255;
            outsideB = 0;
        } else {
            outsideR = this.borderRed.intValue();
            outsideG = this.borderGreen.intValue();
            outsideB = this.borderBlue.intValue();
        }
        final int outsideA = (insideColor >> 24 & 255);
    ...
    private int getStringWidth(String string) {
        return (int) Math.ceil(FontUtils.getStringWidth(string, this.customFont.booleanValue()));
    }
}
