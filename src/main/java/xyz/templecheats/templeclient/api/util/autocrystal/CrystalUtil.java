...
public class CrystalUtil {
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos, String server, boolean ignoreCrystals) {
        if(notValidBlock(mc.world.getBlockState(blockPos).getBlock())) return false;
        
        BlockPos posUp = blockPos.up();
        
        if(!server.equalsIgnoreCase("1.12")) {
        ...
        AxisAlignedBB box = new AxisAlignedBB(
                ...
                posUp.getX() + 1.0, posUp.getY() + (server.equalsIgnoreCase("crystalpvp.cc") ? 1.0 : 2.0), posUp.getZ() + 1.0
        );
    ...
    public static List<BlockPos> findCrystalBlocks(float placeRange, String server) {
    ...
    public static List<BlockPos> findCrystalBlocksExcludingCrystals(float placeRange, String server) {
        return EntityUtil.getSphere(mc.player.getPosition(), placeRange, (int) placeRange, false, true, 0)
                         .stream()
